...
> 書込装置選択 の選択は不要。専用書込装置設定が自動設定される。\
> この製品は EEPROM 等の特殊領域をオンボード書換器で書き換えることは出来ない。
外部書換器を使える設計でもないため、
それに対応するスケッチを記述して直接操作しなければならない。
...
- v0.2.9 (23/12/08)
  - ブートローダー全体を独自のArduino上位互換動作コードに変更。（`Optiboot`由来ソースコードを除去）
  - 同、CRCSCAN機能用CRC16検査値付与。
