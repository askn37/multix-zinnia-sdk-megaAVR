...
> [AVR_DA/DB/DD/DU/EA/EB系統用ブートローダー]（boot_dx/boot_ex）は[こちら](https://github.com/askn37/multix-zinnia-sdk-modernAVR/tree/main/bootloaders)
...
RxDピンの内蔵プルアップ抵抗を有効にする。既定値は無効。通常は送信側がプッシュプル動作を行うので指定の必要はない。一部のハードウェア（USBシリアル変換器）はこれを有効にしないと最初の受信動作に入れないことがある。
